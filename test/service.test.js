...
test('lists only receivables that have been registered on-chain', async () => {
  ...
  const service = new CessioService({ repository, network: { chainId: 968, settlementTokenAddress: '0x2222222222222222222222222222222222222222' }, underwriter: new DeterministicUnderwriter(), clock: () => new Date('2026-08-10T00:00:00Z') });
  const assessment = await service.createAssessment(invoice);
  const receivable = await service.createReceivable({ assessmentId: assessment.id, originatorWallet: invoice.originatorWallet, settlementToken: '0x2222222222222222222222222222222222222222', chainId: 968, requestedFundingAmount: '500.00' });
...
test('does not list receivables from another active network', async () => {
  const repository = new MemoryRepository();
  const service = new CessioService({
    repository,
    network: { chainId: 677, settlementTokenAddress: '0x3333333333333333333333333333333333333333' },
    underwriter: new DeterministicUnderwriter(),
    clock: () => new Date('2026-08-10T00:00:00Z')
  });
  const assessment = await service.createAssessment(invoice);
  const receivable = await service.createReceivable({ assessmentId: assessment.id, originatorWallet: invoice.originatorWallet, settlementToken: '0x2222222222222222222222222222222222222222', chainId: 968, requestedFundingAmount: '500.00' });
  await service.addChainEvent(receivable.id, { type: 'receivable_registered', txHash: `0x${'f'.repeat(64)}`, chainId: 968, contractAddress: '0x2222222222222222222222222222222222222222', chainReceiptId: 1 });
  assert.equal((await service.listReceivables()).length, 0);
});
